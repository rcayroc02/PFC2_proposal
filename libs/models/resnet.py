...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 ...
                 pooling_type='avg', pretrained_weight=None):
        ...
        # Load other pretrained weight
        if pretrained_weight is not None:
            print('load pretrained weight from {}'.format(pretrained_weight))
            self.load_pretrained_weight(resnet, pretrained_weight)
        
        self.base = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool, 
            resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4)
        # self.gap = nn.AdaptiveAvgPool2d(1) # Vanilla model
        self.gap = build_pooling_layer(pooling_type) # follow cluster-contrast
    ...
    def load_pretrained_weight(self, model, pretrained_weight):
        weight = torch.load(pretrained_weight)
        if 'lup_moco_r50.pth' in pretrained_weight:
            model.load_state_dict(weight, strict=False) # LUPerson pretrain only provides trainable params
        else:
