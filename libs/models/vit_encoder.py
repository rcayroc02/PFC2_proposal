...
class OverlappedPartPool2d(nn.Module):
    ...
    def forward(self, x):
        """
        x: (B, C, H, W)
        returns: (B, C, num_parts)
        """
        B, C, H, W = x.shape
        part_feats = []
        stripe_h = H // self.num_parts
        overlap = int(self.overlap_ratio * stripe_h)

        for i in range(self.num_parts):
            start = max(0, i * stripe_h - overlap)
            end = min(H, (i + 1) * stripe_h + overlap)
            patch = x[:, :, start:end, :]
            pooled = F.adaptive_avg_pool2d(patch, (1, 1))
...
class TMGF(nn.Module):
    ...
    def __init__(self, arch, img_size, sie_coef, camera_num, view_num, stride_size, drop_path_rate, drop_rate, attn_drop_rate,
        ...
        overlap_ratio = 0.2
        for i, g in enumerate(self.granularities):
            setattr(self, f'b{i+1}_pool', OverlappedPartPool2d(
                num_parts=g,
