...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), has_early_feature=False, sie_coef=1.0,
                 gem_pool=False, stem_conv=False, enable_early_norm=False, **kwargs):
        super().__init__()
        self.enable_early_norm = enable_early_norm
        self.num_classes = num_classes
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
        self.has_early_feature = has_early_feature
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.cam_num > 0 and self.view_num > 0:
            x = x + self.pos_embed + self.sie_coef * self.sie_embed[camera_id * self.view_num + view_id]
        elif self.cam_num > 0:
            x = x + self.pos_embed + self.sie_coef * self.sie_embed[camera_id]
        elif self.view_num > 0:
            x = x + self.pos_embed + self.sie_coef * self.sie_embed[view_id]
        else:
            x = x + self.pos_embed

        x = self.pos_drop(x)

        if self.has_early_feature:
            for blk in self.blocks[:-1]:
                x = blk(x)
            if self.enable_early_norm:
                x = self.norm(x)
            return x
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,has_early_feature=False,sie_xishu=1.5,
                                   enable_early_norm=False, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,\
        camera=camera, view=view, drop_path_rate=drop_path_rate, sie_coef=sie_xishu, has_early_feature=has_early_feature, enable_early_norm=enable_early_norm, **kwargs)

    return model

def vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0, has_early_feature=False, sie_xishu=1.5,
                                    enable_early_norm=False, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, drop_path_rate=drop_path_rate,\
        camera=camera, view=view, sie_coef=sie_xishu, has_early_feature=has_early_feature, enable_early_norm=enable_early_norm, **kwargs)
    model.in_planes = 384
